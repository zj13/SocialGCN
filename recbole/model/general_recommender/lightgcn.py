...
class LightGCN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        '''self.attlayer1 = torch.nn.Linear(self.latent_dim * 3, self.latent_dim)
        self.attlayer2 = torch.nn.Linear(self.latent_dim * 3, self.latent_dim)
    ...
    def forward(self):
        ...
        embs_in = torch.cat((embeddings_list[0], embeddings_list[1],embeddings_list[2]), 1)
        embs_in_user,embs_in_item = torch.split(embs_in,[self.n_users, self.n_items])
        embs_out1_user = self.attlayer1(embs_in_user)
        embs_out2_user = self.attlayer2(embs_in_user)
        embs_out3_user = self.attlayer3(embs_in_user)
        embs_out_user = torch.stack([embs_out1_user, embs_out2_user,embs_out3_user], dim=1)
        embs_out_soft_user=torch.nn.functional.softmax(embs_out_user, dim=1)

        embs_out1_item = self.attlayer4(embs_in_item)
        embs_out2_item = self.attlayer5(embs_in_item)
        embs_out3_item = self.attlayer6(embs_in_item)
        embs_out_item = torch.stack([embs_out1_item, embs_out2_item,embs_out3_item], dim=1)
        embs_out_soft_item = torch.nn.functional.softmax(embs_out_item, dim=1)

        embs_out_soft=torch.cat((embs_out_soft_user,embs_out_soft_item), 0)

        embs_in2=torch.stack([embeddings_list[0], embeddings_list[1], embeddings_list[2]], dim=1)
        attout = torch.multiply(embs_in2,embs_out_soft)
        lightgcn_all_embeddings =torch.sum(attout,dim=1)

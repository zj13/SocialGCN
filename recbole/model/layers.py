...
class MyattLayer(nn.Module):
    def __init__(self, in_dim, att_dim):
        super(MyattLayer, self).__init__()
        self.in_dim = in_dim
        self.att_dim = att_dim
        self.w = torch.nn.Linear(in_features=in_dim,out_features=att_dim,bias=False)

    def forward(self,inembeddings):
        attsignal=self.w(inembeddings)
        attsignal = fn.relu(attsignal)
        attsignal = torch.sum(attsignal, dim=2)
        attsignal = fn.softmax(attsignal,dim=1) # [batch_size, M]
        return attsignal
